...
class FirewallRuleManager:
    ...
    def save_path(self, path):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(''' INSERT OR IGNORE INTO executable_paths (path, timestamp) VALUES (?, ?) ''', (path, datetime.now().isoformat()))
            self.conn.commit()
            return True
    ...
    def get_latest_path(self):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(''' SELECT path FROM executable_paths ORDER BY timestamp DESC LIMIT 1 ''')
    ...
    def create_firewall_rule(self, rule_name="RawYuanShen"):
        ...
        ps_command = f'New-NetFirewallRule -DisplayName "{rule_name}" -Direction Outbound -Program "{program_path}" -Action Block'
        ...
        try:
            ...
            return {"success": False, "error": f"创建防火墙规则失败！"}
    ...
    def delete_firewall_rule(self, rule_name="RawYuanShen"):
        ...
        try:
            ...
            return {"success": False, "error": f"删除防火墙规则失败或不存在相关规则！"}
        except Exception as e:
            return {"success": False, "error": f"删除防火墙规则失败或不存在相关规则！"}
...
if __name__ == '__main__':
    ...
    # 删除防火墙规则示例
    delete_result = firewall_manager.delete_firewall_rule()
    if delete_result["success"]:
        print("\x1b[93m成功删除防火墙规则!\x1b[0m")
    else:
        print("\x1b[91m错误:\x1b[0m", delete_result["error"])
